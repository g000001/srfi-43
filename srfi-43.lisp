...
(define-function (nonneg-int? x)
  (and (integer? x)
       (not (negative? x))))

(define-function (between? x y z)
  (and (<  x y)
       (<= y z)))

(define-function (unspecified-value) :undef)
...
;++ be reused.
(define-function (vectors-ref vectors i)
  (mapcar (lambda (v) (vector-ref v i)) vectors))
...
;;;   error was signalled.
(define-function (check-type pred? value callee)
...
(define-function (check-index vec index callee)
...
(define-function (check-indices vec start start-name end end-name callee)
  (let ((lose (lambda things
    ...
    (cond ((> start end)
           ...
           (receive (new-start new-end)
                    (funcall lose `(,end-name < ,start-name))
             (check-indices vec
          ...
          ((< start 0)
           (check-indices vec
                          (funcall lose `(,start-name < 0))
                          start-name
                          end end-name
                          callee))
          ((>= start (vector-length vec))
           (check-indices vec
                          (funcall lose `(,start-name > len)
...
(define-function (vector-parse-start+end vec args start-name end-name callee)
...
(define-function %smallest-length
...
;;;   reverse order.
(define-function %vector-reverse-copy!
...
(define-function %vector-reverse!
...
(define-function %vector-fold1
  (letrec ((loop (lambda (kons knil len vec i)
                   (if (= i len)
                       knil
                       (loop kons
                             (funcall kons i knil (vector-ref vec i))
                             len vec (+ i 1))))))
...
;;;     (F <index> <elt>) -> elt'
(define-function %vector-map1!
  (letrec ((loop (lambda (f target vec i)
                   (if (zero? i)
                       target
                       (let ((j (- i 1)))
                         (vector-set! target j
                                      (funcall f j (vector-ref vec j)))
                         (loop f target vec j))))))
...
(define-function vector-unfold
  (letrec ((tabulate!                   ; Special zero-seed case.
            (lambda (f vec i len)
              (cond ((< i len)
                     (vector-set! vec i (funcall f i))
           ...
           (unfold2+!                   ; Slower variant for N seeds.
            (lambda (f vec i len seeds)
              (if (< i len)
                  (destructuring-bind (elt . new-seeds)
                                      (multiple-value-list (apply f i seeds))
                    (vector-set! vec i elt)
...
(define-function vector-unfold-right
  (letrec ((tabulate!
            (lambda (f vec i)
              (cond ((>= i 0)
                     (vector-set! vec i (funcall f i))
...
(define-function (vector-copy vec . args)
...
(define-function (vector-copy..parse-args vec args)
  (labels ((parse-args (start end n fill)
...
(define-function (vector-reverse-copy vec . maybe-start+end)
...
;;;   new vector.
(define-function (vector-append . vectors)
...
;;;   is 0, and #F if not.
(define-function (vector-empty? vec)
...
;;;   specified.
(define-function (vector= elt=? . vectors)
             ...
             (let ((vec1 (check-type #'vector? (car vecs) #'vector=))
               ...
               (or (null? vec2+)
                   (and (binary-vector= elt=? vec1 (car vec2+))
                        (loop vec2+)))))))))
(define-function (binary-vector= elt=? vector-a vector-b)
...
(define-function vector-ref #'vector-ref)

;;; (VECTOR-LENGTH <vector>) -> exact, nonnegative integer
;;;   [R5RS] Return the length of VECTOR.
(define-function vector-length #'vector-length)
...
(define-function (vector-fold kons knil vec . vectors)
...
;;; useful elsewhere.
(define-function vector-fold-right
  (letrec ((loop1 (lambda (kons knil vec i)
                    (if (negative? i)
                        knil
                        (loop1 kons (funcall kons i knil (vector-ref vec i))
                               vec
                               (- i 1)))))
...
(define-function (vector-map f vec . vectors)
...
;;;   the vectors.
(define-function vector-for-each
  (letrec ((for-each1
            (lambda (f vec i len)
              (cond ((< i len)
                     (funcall f i (vector-ref vec i))
...
;;;   reached, return #F.
(define-function (vector-index pred? vec . vectors)
...
(define-function (vector-index-right pred? vec . vectors)
...
(define-function (vector-skip-right pred? vec . vectors)
  (vector-index/skip-right (lambda elts (not (apply pred? elts)))
                           vec vectors
                           #'vector-index-right))

(define-function vector-index/skip-right
...
;;;   value; otherwise, when the shortest vector runs out, return 'nil.
;;;   The iteration and order of application of PRED? across elements
;;;   is of the vectors is strictly left-to-right.
(define-function vector-any
  (letrec ((loop1 (lambda (pred? vec i len len-1)
                    (and (not (= i len))
                         (if (= i len-1)
                             (funcall pred? (vector-ref vec i))
                             (or (funcall pred? (vector-ref vec i))
                                 (loop1 pred? vec (+ i 1)
                                        len len-1))))))
...
;;;   should ever return 'nil, immediately stop and return 'nily; otherwise,
...
;;;   The iteration and order of application of PRED? across elements
;;;   is of the vectors is strictly left-to-right.
(define-function vector-every
  (letrec ((loop1 (lambda (pred? vec i len len-1)
                    (or (= i len)
                        (if (= i len-1)
                            (funcall pred? (vector-ref vec i))
                            (and (funcall pred? (vector-ref vec i))
                                 (loop1 pred? vec (+ i 1)
                                        len len-1))))))
           (loop2+ (lambda (pred? vectors i len len-1)
                     (or (= i len)
...
(define-function vector-fill!
  (let ((%vector-fill! #'cl:fill))   ; Take the native one, under
...
(define-function (vector-copy! target tstart source . maybe-sstart+send)
  (labels ((doit! (sstart send source-length)
...
;;; (VECTOR-REVERSE-COPY! <target> <tstart> <source> [<sstart> <send>])
;;; [wdc] Corrected to allow 0 <= sstart <= send <= (vector-length source).
(define-function (vector-reverse-copy! target tstart source . maybe-sstart+send)
  (labels ((doit! (sstart send source-length)
...
(define-function (vector-reverse! vec . start+end)
...
(define-function vector->list
  (flet ((%vector->list (vector) (coerce vector 'list)))
...
(define-function (reverse-vector->list vec . maybe-start+end)
...
(define-function list->vector
  (flet ((%list->vector (vector) (coerce vector 'list)))
...
(define-function (reverse-list->vector lst . maybe-start+end)
  (let*-optionals maybe-start+end
      ((start 0)
       (end (length lst)))              ; Ugh -- LENGTH
       ...
       (lambda (index l)
         ...
         (cond ((null? l)
               ...
               (:else
                (error "erroneous value"
                       (list #'list? lst)
                       `(while calling ,3'reverse-list->vector)))))))))
